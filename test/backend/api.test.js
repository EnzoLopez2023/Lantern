...
test('health/version expose bounded SQLite, lifecycle, build, and source identity', async () => {
  await withApp(async ({ request, db }) => {
    ...
    const liveness = await body(live)
    ...
    assert.equal(typeof version.instanceId, 'string')
    assert.ok(version.instanceId.length > 0)
    assert.equal(liveness.instanceId, version.instanceId)
    assert.deepEqual(readiness.build, version)
