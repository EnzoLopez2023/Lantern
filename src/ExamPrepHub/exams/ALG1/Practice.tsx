...
export default function Practice() {
  ...
  useEffect(() => {
    ...
    if (mode === 'weak') {
      ...
      // If no due questions, fall back to adaptive to still give 5
    ...
    // intentionally exclude `stats` from deps — we only re-build on mode/filter change,
    // not on every answer recorded (would interrupt the queue mid-session)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [mode, subdomain]);

  const current = queue[index];

  // Reset note state when the current question changes
  useEffect(() => {
    if (current) {
  ...
  const handleNext = () => {
    if (index < queue.length - 1) {
      ...
      // Daily warmup completed — record streak
  ...
  // Compute live summary for the meta-bar
...
// Mode chip helper — used in the mode-strip at the top of Practice
function ModeChip({
