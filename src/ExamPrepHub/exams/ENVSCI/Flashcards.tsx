...
export default function Flashcards() {
  ...
  const handleRate = (rating: FlashcardRating) => {
    ...
    if (index < queue.length - 1) {
      ...
      // End of deck — show summary by setting index past end
    ...
    return (
      <Box sx={{ pb: 6 }}>
        <Paper elevation={0} sx={{ p: 4, backgroundColor: CARD_BG, border: `1px solid ${BORDER}`, borderRadius: 2, textAlign: 'center' }}>
          ...
          {mode === 'due' && (
            <Button
              sx={{ mt: 2, color: ACCENT, textTransform: 'none' }}
              onClick={() => setMode('all')}
            >
              Browse all cards
            </Button>
    ...
    <Box sx={{ pb: 6 }}>
        ...
        <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.5} sx={{ alignItems: { xs: 'flex-start', md: 'center' }, justifyContent: 'space-between' }}>
          <Stack direction="row" spacing={1} sx={{ flexWrap: 'wrap', gap: 1 }}>
            <Chip
              ...
              label={`All cards (${flashcards.length})`}
              onClick={() => setMode('all')}
            ...
            />
            <Chip
              label={`Due now (${dueCount})`}
              onClick={() => setMode('due')}
              variant={mode === 'due' ? 'filled' : 'outlined'}
          ...
          <Stack direction="row" spacing={1} sx={{ alignItems: 'center', flexWrap: 'wrap', gap: 1 }}>
            <Chip
              size="small"
              label={`${reviewedCards}/${flashcards.length} seen`}
              variant="outlined"
              sx={{ borderColor: BORDER, color: TEXT_SEC }}
            />
            {masteredCards > 0 && (
              <Chip
                size="small"
                label={`${masteredCards} mastered`}
                sx={{ backgroundColor: alpha(GOOD, 0.18), color: GOOD, fontWeight: 700 }}
              />
            )}
          </Stack>
        </Stack>
      ...
      >
        <Chip
          ...
          sx={{
            mb: 2,
            backgroundColor: flipped ? alpha(ACCENT, 0.18) : alpha(ACCENT, 0.1),
            color: ACCENT,
            fontWeight: 800,
            letterSpacing: 1,
            fontSize: '0.65rem',
            ...
            return (
              <Button
                ...
                sx={{
                  ...
                  textTransform: 'none',
                  minWidth: 130,
                  '&:hover': { backgroundColor: alpha(color, 0.08), borderColor: color },
                }}
              >
                {r.emoji} {r.label}
              </Button>
      ...
      {!flipped && (
        <Typography variant="caption" sx={{ display: 'block', textAlign: 'center', color: TEXT_SEC, mt: 2 }}>
