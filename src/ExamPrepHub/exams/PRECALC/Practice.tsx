...
export default function Practice() {
  ...
  const startDailyWarmup = () => { setMode('daily'); };
  ...
  return (
    <Box sx={containerSx}>
      <Paper elevation={0} sx={{ p: 2, mb: 2, backgroundColor: CARD_BG, border: `1px solid ${doneToday ? GOOD : ACCENT}`, borderLeft: `4px solid ${doneToday ? GOOD : ACCENT}`, borderRadius: 2 }}>
          ...
          <Box>
            <Stack direction="row" spacing={1} sx={{ alignItems: 'center', mb: 0.5 }}>
              ...
              {streak.currentStreak > 0 && (
                <Chip size="small" label={`${streak.currentStreak}-day streak`} sx={{ backgroundColor: `${ACCENT}22`, color: ACCENT, fontWeight: 700 }} />
              )}
            </Stack>
            <Typography variant="caption" sx={{ color: TEXT_SEC }}>
              {doneToday
                ...
                : `5 questions, sourced from your spaced-repetition due queue.`}
            </Typography>
          </Box>
          {!doneToday && (
            <Button variant="contained" onClick={startDailyWarmup} sx={{ backgroundColor: ACCENT, '&:hover': { backgroundColor: ACCENT, opacity: 0.9 }, fontWeight: 700, textTransform: 'none', flexShrink: 0 }}>
      ...
      <Paper elevation={0} sx={{ p: 2, mb: 2, backgroundColor: CARD_BG, border: `1px solid ${BORDER}`, borderRadius: 2 }}>
        <Stack direction="row" spacing={1} sx={{ flexWrap: 'wrap', gap: 1, mb: 1.5 }}>
          ...
          <ModeChip id="due" label={`Due review (${dueCount})`} current={mode} setMode={setMode} accent={ACCENT} border={BORDER} text={TEXT_PRI} tooltip="Spaced repetition — questions whose next-review date has arrived" />
          <ModeChip id="bookmarks" label={`Bookmarks (${bookmarks.size})`} current={mode} setMode={setMode} accent={ACCENT} border={BORDER} text={TEXT_PRI} tooltip="Questions you've starred to revisit" />
        </Stack>
          ...
          <Stack direction="row" spacing={1.5} sx={{ alignItems: 'center', flexWrap: 'wrap', gap: 1 }}>
            {mode === 'browse' && (
              <FormControl size="small" sx={{ minWidth: 220 }}>
                ...
                <Select value={subdomain} label="Filter by subdomain" onChange={(e) => setSubdomain(e.target.value)} sx={{ color: TEXT_PRI, '.MuiOutlinedInput-notchedOutline': { borderColor: BORDER } }}>
                  <MenuItem value="all">All ({questions.length})</MenuItem>
                  {subdomains.map(sd => (<MenuItem key={sd} value={sd}>{sd}</MenuItem>))}
                </Select>
              </FormControl>
            )}
          </Stack>
          {overallAcc !== null && (
            <Chip label={`${(overallAcc * 100).toFixed(0)}% all-time (${overallAttempted})`} sx={{ backgroundColor: overallAcc >= 0.75 ? `${GOOD}22` : `${BAD}22`, color: overallAcc >= 0.75 ? GOOD : BAD, fontWeight: 700 }} />
          )}
        </Stack>
        ...
        {weakestVisible.length > 0 && (
          <Stack direction="row" spacing={1} sx={{ mt: 1.5, alignItems: 'center', flexWrap: 'wrap', gap: 1 }}>
            ...
            {weakestVisible.slice(0, 4).map(s => (
              <Chip key={s.subdomain} size="small" label={`${s.subdomain} (${(s.accuracy * 100).toFixed(0)}%)`} onClick={() => { setMode('browse'); setSubdomain(s.subdomain); }} sx={{ borderColor: BAD, color: BAD, backgroundColor: 'transparent', border: `1px solid ${BAD}`, cursor: 'pointer' }} />
            ))}
          </Stack>
        )}
      </Paper>
      ...
      <Box sx={{ mb: 2 }}>
        <Stack direction="row" sx={{ alignItems: 'center', justifyContent: 'space-between', mb: 0.5 }}>
          <Typography variant="caption" sx={{ color: TEXT_SEC }}>Question {index + 1} of {queue.length}{mode === 'browse' && subdomain !== 'all' && <span> · {subdomain}</span>}</Typography>
          <Typography variant="caption" sx={{ color: TEXT_SEC }}>{current.difficulty} · {isMulti ? 'multi-select' : 'single answer'}</Typography>
        </Stack>
        <LinearProgress variant="determinate" value={((index + 1) / queue.length) * 100} sx={{ height: 6, borderRadius: 2, backgroundColor: `${ACCENT}22`, '& .MuiLinearProgress-bar': { backgroundColor: ACCENT } }} />
      </Box>
      ...
      <Paper elevation={0} sx={{ p: 3, backgroundColor: CARD_BG, border: `1px solid ${BORDER}`, borderRadius: 2 }}>
        ...
        <Stack spacing={1} sx={{ mb: 2 }}>
          {current.options.map((opt, i) => {
            ...
            } else if (isSelected) { borderColor = ACCENT; bgColor = `${ACCENT}14`; }
            return (
              <Box key={i} onClick={() => toggleOption(i)} sx={{ p: 1.5, borderRadius: 1.5, border: `1px solid ${borderColor}`, backgroundColor: bgColor, cursor: revealed ? 'default' : 'pointer', display: 'flex', alignItems: 'flex-start', gap: 1, transition: 'all 0.12s', '&:hover': revealed ? {} : { borderColor: ACCENT } }}>
                <Box sx={{ width: 22, height: 22, flexShrink: 0, borderRadius: isMulti ? '4px' : '50%', border: `2px solid ${isSelected || (revealed && isAnswer) ? (revealed && isAnswer ? GOOD : ACCENT) : BORDER}`, backgroundColor: isSelected ? (revealed ? (isAnswer ? GOOD : BAD) : ACCENT) : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: '0.85rem', fontWeight: 700 }}>
                  {isSelected ? (isMulti ? '✓' : '●') : ''}
                </Box>
        ...
        {!revealed && (
          <>
            ...
            <Typography variant="caption" sx={{ color: TEXT_SEC, display: 'block', mb: 1, fontWeight: 600 }}>Before revealing — how confident are you?</Typography>
            <Stack direction="row" spacing={1} sx={{ flexWrap: 'wrap', gap: 1, mb: 2 }}>
              {CONFIDENCE_OPTIONS.map(opt => (
                <Chip key={opt.id} label={`${opt.emoji} ${opt.label}`} onClick={() => setConfidence(opt.id)} variant={confidence === opt.id ? 'filled' : 'outlined'} sx={confidence === opt.id ? { backgroundColor: ACCENT, color: '#fff', fontWeight: 700 } : { borderColor: BORDER, color: TEXT_PRI }} />
              ))}
            </Stack>
            <Stack direction="row" spacing={1}>
              <Button variant="contained" onClick={handleReveal} disabled={selected.length === 0 || !confidence} sx={{ backgroundColor: ACCENT, '&:hover': { backgroundColor: ACCENT, opacity: 0.9 } }}>Reveal answer</Button>
        ...
        {revealed && (
          <>
            ...
            <Paper variant="outlined" sx={{ p: 1.5, mb: 1.5, borderColor: BORDER, backgroundColor: isDark ? '#1E0F06' : '#FBF7F2' }}>
              <Typography variant="caption" sx={{ color: TEXT_SEC, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5 }}>Why</Typography>
              <Typography sx={{ color: TEXT_PRI, fontSize: '0.92rem', mt: 0.5, lineHeight: 1.6 }}>{current.explanation}</Typography>
            </Paper>

            <Box sx={{ mb: 1.5 }}>
              <Button size="small" startIcon={<NoteIcon fontSize="small" />} endIcon={noteOpen ? <ExpandLessIcon fontSize="small" /> : <ExpandMoreIcon fontSize="small" />} onClick={() => setNoteOpen(o => !o)} sx={{ color: noteText ? ACCENT : TEXT_SEC, textTransform: 'none', fontSize: '0.8rem' }}>
                {noteText ? 'Edit note' : 'Add a personal note'}{noteText && !noteOpen && ' — saved'}
              </Button>
              {noteOpen && (
                <TextField fullWidth multiline minRows={2} maxRows={6} value={noteText} onChange={e => setNoteText(e.target.value)} onBlur={handleNoteSave} placeholder="Why I missed this · the mnemonic that worked · etc." size="small" sx={{ mt: 1, '& .MuiInputBase-root': { fontSize: '0.88rem' } }} />
              )}
            </Box>

            <Stack direction="row" spacing={1}>
              <Button variant="contained" onClick={handleNext} disabled={index >= queue.length - 1 && !(mode === 'daily' && !doneToday)} sx={{ backgroundColor: ACCENT, '&:hover': { backgroundColor: ACCENT, opacity: 0.9 } }}>
                {mode === 'daily' && index === queue.length - 1 ? 'Finish daily warmup' : 'Next question'}
              </Button>
              <Button variant="text" startIcon={<ResetIcon />} onClick={() => { setIndex(0); setSelected([]); setConfidence(null); setRevealed(false); }} sx={{ color: TEXT_SEC }}>Restart queue</Button>
            </Stack>
          </>
        )}
      </Paper>
...
function ModeChip({ id, label, current, setMode, accent, border, text, tooltip }: { id: Mode; label: string; current: Mode; setMode: (m: Mode) => void; accent: string; border: string; text: string; tooltip?: string; }) {
  const active = current === id;
  const chip = (<Chip label={label} onClick={() => setMode(id)} variant={active ? 'filled' : 'outlined'} sx={active ? { backgroundColor: accent, color: '#fff', fontWeight: 700 } : { borderColor: border, color: text }} />);
