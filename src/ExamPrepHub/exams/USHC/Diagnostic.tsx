...
// Map drill accuracy to estimated exam score (0..1000).
// Pass is 700. Tuned conservatively: 75% drill accuracy → ~720 estimated.
function accuracyToScore(acc: number): number {
  // Linear, with a small floor to avoid 0% mapping to 0 — Microsoft scales.
  // 0% drill = 350, 50% drill = 600, 75% drill = 720, 90% drill = 870, 100% drill = 950
  const clamped = Math.max(0, Math.min(1, acc));
  return Math.round(350 + clamped * 600);
}

// Domain-weighted confidence estimate
// Domain 1 = 22% of exam (mid of 20-25), Domain 2 = 78%
function weightedAccuracy(d1: number, d2: number): number {
  return 0.22 * d1 + 0.78 * d2;
}
...
export default function Diagnostic() {
  ...
  const verdictMeta = {
    ready:    { icon: <ReadyIcon />,   color: GOOD, label: "You're ready",          blurb: 'Estimated score sits comfortably above pass. Schedule the exam this week.' },
    almost:   { icon: <AlmostIcon />,  color: WARN, label: 'Almost there',          blurb: 'Estimated score is at or just above pass. Drill weak areas before booking.' },
    'not-yet':{ icon: <NotYetIcon />,  color: BAD,  label: 'Not yet — keep going',  blurb: 'Estimated score below pass. Focus on the weak subdomains below.' },
    unknown:  { icon: <InsightsIcon/>, color: ACCENT, label: 'Need more data',      blurb: 'Answer at least 10 questions in Drill mode to get a real readiness estimate.' },
  }[verdict];

  // Shell handles maxWidth + horizontal padding; we just space the bottom.
  ...
  return (
    <Box sx={containerSx}>
      ...
      {!hasEnoughData ? (
        <Paper elevation={0} sx={{ p: 3, backgroundColor: CARD_BG, border: `1px solid ${BORDER}`, borderRadius: 2 }}>
          ...
          <Typography sx={{ color: TEXT_PRI, fontSize: '0.92rem', lineHeight: 1.7, mb: 1.5 }}>
            Go to the <strong>Drill</strong> tab and answer at least 10 questions. Each answer gets weighted by its
            exam domain (Domain 1 ≈ 22%, Domain 2 ≈ 78%). Your weighted accuracy gets mapped to an estimated
            scaled score on Microsoft's 0–1000 scale (pass = 700).
          </Typography>
          <Typography sx={{ color: TEXT_SEC, fontSize: '0.85rem', lineHeight: 1.6 }}>
            The estimate is intentionally conservative — Microsoft's real scoring includes question difficulty and
            sub-skill weighting we don't see. If this dashboard says you're ready, you're <em>actually</em> ready.
          </Typography>
        </Paper>
      ) : (
        <>
          ...
          <Paper elevation={0} sx={{ p: 3, mb: 2, backgroundColor: CARD_BG, border: `1px solid ${BORDER}`, borderRadius: 2 }}>
            ...
            <Stack spacing={2}>
              ...
              <DomainRow
                ...
                weight="≈78% of exam"
                acc={domain2.accuracy}
                attempted={domain2.attempted}
                correct={domain2.correct}
                colors={{ ACCENT, BORDER, TEXT_PRI, TEXT_SEC, GOOD, WARN, BAD }}
              />
            </Stack>
          ...
          <Paper elevation={0} sx={{ p: 3, mb: 2, backgroundColor: CARD_BG, border: `1px solid ${BORDER}`, borderRadius: 2 }}>
            ...
            <Typography variant="caption" sx={{ color: TEXT_SEC, display: 'block', mb: 2 }}>
              Sorted weakest first. Items with fewer than 3 attempts are shown gray — drill them in the Drill tab to get a real read.
            </Typography>
            <Stack spacing={1.5}>
              {summary.map(s => {
      ...
      {/* === Analytics block (sandbox attempts) === */}
  ...
  if (attempts.length === 0) {
    return (
      <Paper elevation={0} sx={{ p: 3, mt: 2, backgroundColor: CARD_BG, border: `1px solid ${BORDER}`, borderRadius: 2 }}>
        <Typography variant="h6" sx={{ fontWeight: 700, color: TEXT_PRI, mb: 1 }}>Sandbox analytics</Typography>
        <Typography variant="body2" sx={{ color: TEXT_SEC }}>
          Take a sandbox exam to populate attempt history, time-per-question analytics, and the activity heatmap.
        </Typography>
      </Paper>
    );
  }
  ...
  return (
    <Box sx={{ mt: 2 }}>
      ...
      <Paper elevation={0} sx={{ p: 3, mb: 2, backgroundColor: CARD_BG, border: `1px solid ${BORDER}`, borderRadius: 2 }}>
        ...
        <Typography sx={{ fontWeight: 700, color: TEXT_PRI, fontSize: '0.9rem', mb: 1 }}>Latest attempt — time per question</Typography>
        <TimePerQuestion attempt={attempts[0]} accent={ACCENT} warn={WARN} good={GOOD} bad={BAD} muted={TEXT_SEC} text={TEXT_PRI} border={BORDER} />
      </Paper>
...
function TimePerQuestion({
  attempt, accent, warn, good, bad, muted, text, border,
}: {
  attempt: ReturnType<typeof loadAttempts>[0];
  accent: string; warn: string; good: string; bad: string; muted: string; text: string; border: string;
}) {
  const times = attempt.perQuestion.map(q => q.timeSec);
  const maxTime = Math.max(...times, 60); // floor at 60s for sane scale
  const avg = times.length === 0 ? 0 : times.reduce((a, b) => a + b, 0) / times.length;

  const fastThreshold = avg * 0.5;
  const slowThreshold = avg * 1.75;
  ...
  return (
    <Box>
      <Stack direction="row" spacing={1} sx={{ mb: 1, flexWrap: 'wrap', gap: 1 }}>
        <Chip
          size="small"
          label={`Avg ${avg.toFixed(0)}s per question`}
          sx={{ backgroundColor: alpha(accent, 0.15), color: accent, fontWeight: 700 }}
        />
        <Chip
          size="small"
          label={`Total ${attempt.timeSpentSec}s`}
          variant="outlined"
          sx={{ borderColor: border, color: muted }}
        />
        <Tooltip title="Questions you spent much less time on than average — were you rushing?">
          <Chip size="small" label={`${times.filter(t => t > 0 && t < fastThreshold).length} rushed`} sx={{ backgroundColor: alpha(warn, 0.15), color: warn, fontWeight: 700 }} />
        </Tooltip>
        <Tooltip title="Questions you spent way more time on than average — overthinking or genuinely tough">
          <Chip size="small" label={`${times.filter(t => t > slowThreshold).length} slow`} sx={{ backgroundColor: alpha(bad, 0.15), color: bad, fontWeight: 700 }} />
        </Tooltip>
      </Stack>
      <Box sx={{ display: 'flex', alignItems: 'flex-end', gap: '2px', height: 80, p: 0.5, border: `1px solid ${border}`, borderRadius: 1 }}>
        {attempt.perQuestion.map((q, i) => {
          const h = maxTime === 0 ? 0 : (q.timeSec / maxTime) * 70;
          const color = q.correct ? good : bad;
          return (
            <Tooltip
              key={q.questionId}
              title={`Q${i + 1} · ${q.timeSec}s · ${q.correct ? 'correct' : 'wrong'} · ${q.subdomain}`}
            >
              <Box
                sx={{
                  flex: 1,
      ...
      <Typography variant="caption" sx={{ color: muted, mt: 0.5, display: 'block' }}>
        Green = correct, red = wrong. Bar height = seconds spent. Hover for details.
      </Typography>
      <Typography variant="caption" sx={{ color: text, mt: 1, display: 'block', fontSize: '0.78rem' }}>
        Note: time-per-question isn't tracked for case-study sub-questions individually.
      </Typography>
    </Box>
  );
}
...
function ActivityHeatmap({
  ...
  // 12 weeks × 7 days grid (oldest on the left)
  return (
    <Box sx={{ width: '100%', overflowX: 'auto' }}>
