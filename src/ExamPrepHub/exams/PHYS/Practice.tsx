  ...
  return (
    <Box sx={containerSx}>
      <Paper
        elevation={0}
        sx={{
          p: 2, mb: 2, backgroundColor: CARD_BG,
          ...
          <Box>
            <Stack direction="row" spacing={1} sx={{ alignItems: 'center', mb: 0.5 }}>
              ...
              {streak.longestStreak > streak.currentStreak && (
                <Chip size="small" label={`Best: ${streak.longestStreak}`} variant="outlined" sx={{ borderColor: BORDER, color: TEXT_SEC }} />
              )}
            </Stack>
      ...
      <Paper elevation={0} sx={{ p: 2, mb: 2, backgroundColor: CARD_BG, border: `1px solid ${BORDER}`, borderRadius: 2 }}>
        ...
        <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.5} sx={{ alignItems: { xs: 'flex-start', md: 'center' }, justifyContent: 'space-between' }}>
          <Stack direction="row" spacing={1.5} sx={{ alignItems: 'center', flexWrap: 'wrap', gap: 1 }}>
            ...
            {mode === 'daily' && (
              <Chip size="small" label={`Daily warmup · ${index + 1}/${queue.length}`} sx={{ backgroundColor: `${ACCENT}22`, color: ACCENT, fontWeight: 700 }} />
            )}
          </Stack>
          {overallAcc !== null && (
            <Chip
        ...
        {weakestVisible.length > 0 && (
          <Stack direction="row" spacing={1} sx={{ mt: 1.5, alignItems: 'center', flexWrap: 'wrap', gap: 1 }}>
            ...
            {weakestVisible.slice(0, 4).map(s => (
              <Chip key={s.subdomain} size="small" label={`${s.subdomain} (${(s.accuracy * 100).toFixed(0)}%)`}
      ...
      <Paper elevation={0} sx={{ p: 3, backgroundColor: CARD_BG, border: `1px solid ${BORDER}`, borderRadius: 2 }}>
        <Stack direction="row" spacing={1} sx={{ mb: 1, alignItems: 'center', flexWrap: 'wrap', gap: 0.5 }}>
          <Chip size="small" label={current.subdomain} sx={{ backgroundColor: `${ACCENT}22`, color: ACCENT, fontWeight: 700 }} />
          <Box sx={{ flexGrow: 1 }} />
        ...
        <Stack spacing={1} sx={{ mb: 2 }}>
          {current.options.map((opt, i) => {
            ...
            return (
              <Box key={i} onClick={() => toggleOption(i)}
                sx={{
                  p: 1.5, borderRadius: 1.5, border: `1px solid ${borderColor}`, backgroundColor: bgColor,
                  cursor: revealed ? 'default' : 'pointer', display: 'flex', alignItems: 'flex-start', gap: 1,
                  transition: 'all 0.12s', '&:hover': revealed ? {} : { borderColor: ACCENT },
                }}
              >
                <Box sx={{
          ...
          <>
            ...
            <Stack direction="row" spacing={1} sx={{ flexWrap: 'wrap', gap: 1, mb: 2 }}>
              {CONFIDENCE_OPTIONS.map(opt => (
                <Chip key={opt.id} label={`${opt.emoji} ${opt.label}`} onClick={() => setConfidence(opt.id)}
                  variant={confidence === opt.id ? 'filled' : 'outlined'}
                  sx={confidence === opt.id ? { backgroundColor: ACCENT, color: '#fff', fontWeight: 700 } : { borderColor: BORDER, color: TEXT_PRI }}
                />
              ))}
            </Stack>
            <Stack direction="row" spacing={1}>
              <Button variant="contained" onClick={handleReveal} disabled={selected.length === 0 || !confidence}
        ...
        {revealed && (
          <>
            ...
            <Stack direction="row" spacing={1} sx={{ alignItems: 'center', mb: 1.5 }}>
              ...
              {confidence && (
                <Chip size="small" label={`You were ${CONFIDENCE_OPTIONS.find(c => c.id === confidence)?.label.toLowerCase()}`}
                  variant="outlined" sx={{ borderColor: BORDER, color: TEXT_SEC }} />
              )}
            ...
            <Box sx={{ mb: 1.5 }}>
              <Button size="small" startIcon={<NoteIcon fontSize="small" />}
              ...
              {noteOpen && (
                <TextField fullWidth multiline minRows={2} maxRows={6} value={noteText}
                  onChange={e => setNoteText(e.target.value)} onBlur={handleNoteSave}
                  placeholder="Why I missed this · the mnemonic that worked · etc."
                  size="small" sx={{ mt: 1, '& .MuiInputBase-root': { fontSize: '0.88rem' } }} />
              )}
            </Box>

            <Stack direction="row" spacing={1}>
              <Button variant="contained" onClick={handleNext}
                disabled={index >= queue.length - 1 && !(mode === 'daily' && !doneToday)}
                sx={{ backgroundColor: ACCENT, '&:hover': { backgroundColor: ACCENT, opacity: 0.9 } }}>
                {mode === 'daily' && index === queue.length - 1 ? 'Finish daily warmup' : 'Next question'}
              </Button>
              <Button variant="text" startIcon={<ResetIcon />}
...
function ModeChip({
  ...
  id: Mode; label: string; current: Mode; setMode: (m: Mode) => void;
  accent: string; border: string; text: string; tooltip?: string;
}) {
  const active = current === id;
  const chip = (
    <Chip label={label} onClick={() => setMode(id)} variant={active ? 'filled' : 'outlined'}
      sx={active ? { backgroundColor: accent, color: '#fff', fontWeight: 700 } : { borderColor: border, color: text }} />
  );
